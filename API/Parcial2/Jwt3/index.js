...
async function autenticarUsuario(req, res, next) {
    const connection = await mysql.createConnection({host:'localhost', user: 'root', database: 'basedatos_pruebas_api'});
    const [rows, fields] = await connection.execute('SELECT * FROM `usuarios` WHERE `login` = ?', [req.body.login]);
    if(rows.length==0) {
